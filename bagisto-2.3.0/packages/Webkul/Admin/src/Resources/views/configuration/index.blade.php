    ...
    <div class="grid gap-y-8">
        @foreach (system_config()->getItems() as $item)
            <div>
                ...
                <div
                    class="box-shadow max-1580:grid-cols-3 mt-2 grid grid-cols-4 flex-wrap justify-between gap-12 rounded bg-white p-4 dark:bg-gray-900 max-xl:grid-cols-2 max-sm:grid-cols-1">
                    <!-- Menus cards -->
                    @foreach ($item->getChildren() as $key => $child)
                        <a class="flex max-w-[360px] items-center gap-2 rounded-lg p-2 transition-all hover:bg-gray-100 dark:hover:bg-gray-950"
                            ...
                            @if ($icon = $child->getIcon())
                                <img class="h-[60px] w-[60px] dark:mix-blend-exclusion dark:invert"
